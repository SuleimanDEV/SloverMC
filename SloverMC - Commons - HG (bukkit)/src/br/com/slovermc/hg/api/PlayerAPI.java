		...
		if (PlayerAPI.getInstance().getPlayers().length <= 1 && BukkitMain.state == StateEnum.GAME) {
			if (PlayerAPI.getInstance().getPlayers().length == 1) {
				...
				new BukkitRunnable() {
					...
					@Override
					public void run() {
						Bukkit.broadcastMessage("                  §6§lVITÓRIA");
						Bukkit.broadcastMessage("");
						Bukkit.broadcastMessage("  §e" + v.getName() + " §fvenceu a partida!");
						Bukkit.broadcastMessage("  §fRecebeu §650 XP §fe agradecemos pela jogabilidade!");
						Bukkit.broadcastMessage("                §6www.slovermc.com");
						Bukkit.broadcastMessage("");
